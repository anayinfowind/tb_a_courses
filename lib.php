 ...
 */
function block_tb_a_courses_pluginfile($course, $birecord, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $fs = get_file_storage();

    $filename = array_pop($args);
    $filepath = $args ? '/' . implode('/', $args) . '/' : '/';

    if (!$file = $fs->get_file($context->id, 'block_tb_a_courses', 'courseimagedefault', 0, $filepath, $filename)) {
        send_file_not_found();
    }
